<template>
  <section class="page-stack">
    <UiToolbar as="form" @submit.prevent="loadRecords">
      <UiField label="手机号">
      ...
      <ResultMessage :error="!!error">{{ error || summaryText }}</ResultMessage>
    </UiToolbar>

    <DataTable title="关联交易记录" :tag="`${records.length} 条`" min-width="1320px">
      <thead>
        <tr>
          <th>日期</th>
          <th>类型</th>
      ...
      <tbody>
        ...
        <tr v-for="item in records" :key="`${item.businessType}-${item.businessId}`">
          ...
          <td>
            <UiTag :variant="item.businessType === 'SALE' ? 'amber' : ''">
